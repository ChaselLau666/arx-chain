...
- `--height`：固定高度命令。
- `--task`：任务名称。
- `--key_collect`：可选；使用键盘回车开始，但结束方式仍是复位或达到 800 帧。
...
1. 将左右臂都回到复位位置；程序要求 14 维 qpos 接近 0。
2. 终端/语音提示当前 episode 编号。
3. 操作左右夹爪完成准备手势；语音提示 `go`。
4. 从 `go` 开始正式记录三相机、双臂 state 和 action。

如果使用 `--key_collect`，倒计时后按回车开始，不需要准备手势。

## 5. 结束当前 episode

满足任一条件即结束记录：

1. 已记录超过 100 帧后，将左右臂完整回到复位位置；或
2. 达到 `--max_timesteps`，默认 800 帧，约 13.3 秒。

结束后程序不会立即保存，而是显示：

```text
Episode finished: [s]ave, [d]iscard, [q]uit:
```
...
然后按回车。程序同步压缩并写完 HDF5，语音提示 `Save`。确认终端出现 `Saved in ...` 后：
...
```text
Ready for episode N: [n]ext, [q]uit:
```

- 输入 `n` 并回车：进入下一次复位/准备等待。上一条已保存时使用新编号；上一条已丢弃时复用原编号。
- 输入 `q` 并回车：不开始下一次采集，collector 正常退出。

因此，任何下一条 episode 都必须经过操作者显式输入 `n`，不会因为上一条保存完成而自动开始。
...
然后按回车。当前条不保存，collector 正常退出。其他由 `01_collect.sh` 启动的硬件节点仍在运行，需要按安全顺序单独关闭。
