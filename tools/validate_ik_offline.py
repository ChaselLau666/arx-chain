...
questions that decide whether moving IK to the host is viable:
  ...
  2. Does IK on that eef recover the recorded joint angles - converged, and
     with a single solve per frame as a real-time node would do?
...
FK. What the run proves is that the URDF, frames and solver are right; it does
not exercise unreachable or lagged targets, which only live teleop will.
"""
...
def kinematic_urdf(src: Path, dst: Path) -> Path:
    """The vendor URDF points at meshes Placo cannot find; FK needs none."""
    tree = ET.parse(src)
...
def check_fk(arm: Arm, qpos, eef, stride=4):
    """Returns the constant position offset and the Euler convention that fits."""
    ...
    best = min(angles, key=lambda c: np.mean(angles[c]))
    print('  FK vs recorded eef')
    print(f'    position offset FK - eef = {offsets.mean(0).round(4)}  (std {offsets.std(0).max() * 1000:.3f} mm; ~0 means constant)')
...
def check_ik(urdf: Path, placo, qpos, eef, offset, conv, iters: int, dt: float, label: str):
    arm = Arm(urdf, placo)
    ...
    for k in range(s0, s1):
        T = np.eye(4)
        T[:3, :3] = R.from_euler(conv, eef[k, 3:6]).as_matrix()
        T[:3, 3] = eef[k, :3] + offset
...
def main():
    ...
    parser.add_argument('--urdf', type=Path, default=VENDOR_URDF, help='vendor URDF; meshes are stripped into a cached copy')
    ...
    urdf = kinematic_urdf(args.urdf, args.episode.resolve().parent.parent / 'x5_kin.urdf')
    ...
    for arm_name in arms:
        ...
        if span.max() < 5:
            ...
            continue
        arm = Arm(urdf, placo)
        offset, conv = check_fk(arm, qpos, eef)
        check_ik(urdf, placo, qpos, eef, offset, conv, iters=20, dt=0.05, label='converged (20 iterations/frame)')
        check_ik(urdf, placo, qpos, eef, offset, conv, iters=1, dt=1.0 / args.frame_rate, label=f'real-time (1 solve/frame at {args.frame_rate:.0f} Hz)')
